...
export async function createSwiyuCredential(
  credentialData: DiplomaSchema
): Promise<any> {
  const client = createApiClient();

  const response = await client.post('/credentials', {
    metadata_credential_supported_id: ['e-id-how-06-2025'],
...
export async function checkCredentialOfferStatus(
  credentialId: string
): Promise<any> {
  const client = createApiClient();
...
export async function createSwiyuVerification(
  sd_fields: string[]
...
export async function createSwiyuBetaIDVerification(): Promise<{
  verificationId: string;
  verificationURL: string;
}> {
  const client = createVerifierAPIClient();
  const sd_fields_array = [
    'given_name',
    'family_name',
    'document_number',
    'birth_date',
...
export async function checkVerificationStatus(
  ...
  return {
    ...
    disclosedData: response.data.wallet_response,
  };
}
